...
cv_evaluation_df = pd.read_csv('HandCraft/Brain data CV evaluation with TPES.csv')

# Find row number with min of MSE column, name it best_fold
best_fold = cv_evaluation_df['MSE'].idxmin()
print(best_fold)

## Get model from best fold
filename = f"HandCraft/EN_brain_TPES{best_fold}.pkl"
...
for filename in os.listdir(folder_path):
    if 'tr' in filename and 'rep_0_' in filename and 'brain' in filename:
        print(folder_path+filename)
        train_cv=pd.read_csv(folder_path+filename)
    elif 'te' in filename and 'rep_0_' in filename and 'brain' in filename:
        print(folder_path+filename)
        test_cv=pd.read_csv(folder_path+filename)
    elif 'va' in filename and 'rep_0_' in filename and 'brain' in filename:
        print(folder_path+filename)
...
for col in train_cv.columns:
    ...
    if col!= 'outcome' and col!='Family_ID':
...
metrics_df.to_csv("HandCraft/evaluation_metrics_lockbox_brain_alt.csv", index=False)
