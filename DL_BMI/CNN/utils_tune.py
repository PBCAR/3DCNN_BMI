...
# test the model (returns true and predicted values)
    ...
    for _, data in enumerate(dataloader, 0):
        ...
        if cr == 'clx':
            ...
            y_pred = np.concatenate(
                (y_pred, outputs[0].data.cpu().numpy().squeeze())) # removed .squeeze() at the end
...
## need to figure out what this function does?? seems to be training while validating model
def generate_validation_model(cfg):
    ...
    # Regression
        ...
        reduce_on = 'min'
        m_val_acc = 100
        history = pd.DataFrame(columns=['scorename', 'epoch', 'tr_mae', 'tr_ev', 'tr_mse',
    ...
    # Declare optimizer (tune this as well)
    
    if (cfg.opt == "Adam"):
        optimizer = optim.Adam(net.parameters(), lr = cfg.lr, weight_decay = cfg.l2, amsgrad = cfg.ams)
    if (cfg.opt == "SGD"):
        optimizer = optim.SGD(net.parameters(), lr = cfg.lr, momentum = cfg.momentum, weight_decay = cfg.l2)

    # Declare learning rate scheduler
    scheduler = ReduceLROnPlateau(
        optimizer, mode=reduce_on, factor=0.5, patience=7, verbose=True)

    # Batch Dataloader (Training and Validation Data)
    ...
    for epoch in range(cfg.es):
        ...
        if cfg.cr == 'clx':
            ...
            # Log Performance
            history.loc[epoch] = [cfg.scorename, epoch, train_acc,
                                  bal_train_acc, valid_acc, bal_valid_acc, loss]

            # Check for maxima/performance-improvement (e.g. accuracy for classification)
            ...
            # Log Performance
            history.loc[epoch] = [cfg.scorename, epoch, train_mae, train_ev, train_mse, train_r2,
...
def evaluate_test_accuracy(cfg):
    ...
    # Regression
        ...
        # Initialize Log File
        outs = pd.DataFrame(
            columns=['mae_te', 'ev_te', 'mse_te', 'r2_te', 'r_te', 'p_te'])
...
def loadData(cfg, mode):
    ...
    # Batch Dataloader
    prefetch_factor = cfg.nw
    dset = MRIDataset(cfg, mode)
    #if mode=="tr" or mode=="va":
   # 	train_sampler = torch.utils.data.distributed.DistributedSampler(dataset_train)
    ...
    dloader = DataLoader(dset, batch_size=cfg.bs,
                         shuffle=True, num_workers=cfg.nw, drop_last=True, pin_memory=True, prefetch_factor=prefetch_factor, persistent_workers=True) # potential issue with pin memory?
...
def updateML(cfg):
    ...
    cfg.ml = cfg.ml+cfg.mt+'_scorename_'+cfg.scorename+'_tss_'+str(cfg.tss)+'_rep_'+str(cfg.rep)+'_bs_'+str(cfg.bs)+'_lr_' + \
...
def initializeNet(cfg):
    ...
    # Initialize net based on model type (mt, nc)
    if cfg.mt == 'AlexNet3D_Dropout_Regression':
        net = AlexNet3D_Dropout_Regression(num_classes=1, p = cfg.p)
    else:
...
#        weights_filename,  map_location=lambda storage, loc: storage)
    state_dict = torch.load(
        weights_filename,  map_location="cpu")
    #state = net.state_dict()
    #state.update(state_dict)
    net.load_state_dict(state_dict, strict=True) #### note to self - added strict=False
    #net.load_state_dict(torch.load(weights_filename,  map_location=lambda storage, loc: storage))

    return net

#if __name__=='__main__':
#   generate_validation_model()
